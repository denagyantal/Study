...
def greedy_cow_transport(cows,limit = 10):
    ...
    #sort a list and make a copy
    sortedCows = sorted(cows.items(), key=lambda t: t[1], reverse = True)
    ...
    #iterating until cows are left
    while sortedCows != []:
        #picking one cow at the same time
        for key in sortedCows:    
            
            if(totalWeight + cows[key[0]]) <= limit:
                subresult.append(key[0])
                totalWeight += cows[key[0]]
...
def brute_force_cow_transport(cows,limit=10):
    ...
    for part in allPart:
        ...
        if len(test) == len(part): #checks if all parts from part are ok
            usePart.append(part)
...
def compare_cow_transport_algorithms():
    ...
    x = (end1 - start1)/(end - start)
    
    print ("The greedy is %s times faster than the brute force" % x)
